...
- a manual live-output mode for local E2E runs so progress logs or `tqdm` can be observed directly in the terminal without changing the default captured/report behavior, including a cleaner progress-bar-first live path for producer, consumer, and loader output

...
Stage 17A adds the first repository data contract for the raw IoT log CSV schema. Stage 17B adds local contract validation tooling through `scripts/validate_data_contract.py`. These stages do not change producer runtime logic, consumer runtime logic, warehouse-loader runtime logic, Airflow DAG logic, dbt models, Spark jobs, MinIO logic, Terraform logic, or benchmark scripts.
